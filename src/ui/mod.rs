...
pub fn setup_home_view(
    ...
    .with_children(|root_parent| {
        root_parent.spawn(home_menu());

        let row_entity = root_parent.spawn(row_!(
            width: percent(100),
            height: percent(100),
            margin_top: px(10),
        ...
        .with_children(|row_parent| {
            row_parent.spawn(operator_panel(&operator_list));
            row_parent.spawn(console_panel());
            row_parent.spawn(operator_context());
            row_parent.spawn(
                row_!(
